...
for ii=1:5
    ...
    [t,y]   = ode45(@(t,y) ode45ccl(t,y,a_hat_x, m_hat_x, c_hat_x, g, alpha, beta, lambda, gamma_a, freq_x, a_hat_phi, m_hat_phi, c_hat_phi, freq_phi, l, xd_bar, phid_bar,K_phi,K_cl,T,Yusum_x,YYsum_x,Yusum_phi,YYsum_phi), tspan, y0);
...
function dydt=ode45ccl(t,y,a_hat_x, m_hat_x, c_hat_x, g, alpha, beta, lambda, gamma_a, freq_x,a_hat_phi, m_hat_phi, c_hat_phi, freq_phi, l,xd_bar, phid_bar,K_phi,K_cl,T,Yusum_x,YYsum_x,Yusum_phi,YYsum_phi)
dydt=zeros(15,1);
r_x     =y(1);
u_til_x =y(2);
e_x     =y(3);
...
instime=t/0.01;
u_x=u_til_x+ud_x;
U_x=(m_x+m_phi*sind(phi)^2)*dydt(1) + m_phi*sind(phi)*cosd(phi)*phi_dot*r_x -u_til_x+ud_x;

if instime<=T
    
    theta_til_dot_x = Gamma1 .* Y_x' .* r_x + Gamma1*K_phi*((Y_x.')*U_x-Y_x.'*Y_x*theta_hat_x);
    
else
    Yusum_x=Yusum_x+Y_x.'*U_x;
    YYsum_x=YYsum_x+Y_x.'*Y_x;
    theta_til_dot_x = Gamma1 .* Y_x' .* r_x +Gamma1*K_phi*((Y_x.')*U_x - Y_x.'*Y_x*theta_hat_x)+Gamma1*K_cl*(Yusum_x-YYsum_x*theta_hat_x);
end
...
U_phi = m_phi*(l^2)*dydt(9)+ud_phi-u_til_phi;
if instime<=T
    theta_til_dot_phi = Gamma2 .* Y_phi' .* r_phi +Gamma2*K_phi*(Y_phi.'*U_phi - Y_phi.'*Y_phi*theta_hat_phi);
else
    Yusum_phi=Yusum_phi+ Y_phi.'*U_phi;
    YYsum_phi=YYsum_phi+ Y_phi.'*Y_phi;
    theta_til_dot_phi = Gamma2 .* Y_phi' .* r_phi +Gamma2*K_phi*(Y_phi.'*U_phi - Y_phi.'*Y_phi*theta_hat_phi) +Gamma2*K_cl*(Yusum_phi-YYsum_phi*theta_hat_phi);
end
