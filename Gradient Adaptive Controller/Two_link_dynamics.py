...
class dynamics():
    def __init__(self, alpha=0.2*np.ones(2,dtype=np.float32), beta=0.1*np.ones(2, dtype=np.float32), gamma=0.01*np.ones(5,dtype=np.float32)):
        ...
        self.phid=np.array([np.pi/8, np.pi/4],dtype=np.float32)
        self.freq=0.2
        self.a=np.pi/2
    ...
    def getDesiredstate(self,t):
        ...
        phid=np.array([self.phid[0]*sin(2*np.pi*self.freq*t-self.a)-self.b[0],
                     self.phid[1]*sin(2*np.pi*self.freq*t-self.a)+self.b[1]], dtype=np.float32)
        
        #To get the desired angular velocity
        phiDd=np.array([2*np.pi*self.freq*self.phid[0]*cos(2*np.pi*self.freq*t- self.a),
                       2*np.pi*self.freq*self.phid[1]*cos(2*np.pi*self.freq*t- self.a)], dtype=np.float32)
        
        #To get the desired angular acceleration
        phiDDd=np.array([-((2*np.pi*self.freq)**2)*self.phid[0]*sin(2*np.pi*self.freq*t - self.a),
                        -((2*np.pi*self.freq)**2)*self.phid[1]*sin(2*np.pi*self.freq*t - self.a)], dtype=np.float32)
    ...
    def getM(self,m,l,phi):
        ...
        M= np.array([[(m1*l1**2)+m2*((l1**2)+2*l1*l2*c2),(m2*(l1*l2*c2 + l2**2))],
    ...
    def getC(self,m,l,phi,phiD):
        ...
        C=np.array([(-2*m2*l1*l2*s2*phiD[0]*phiD[1]- m2*l1*l2*s2*(phiD[1]**2)),
                    (m2*l1*l2*s2*phi[0]**2)], dtype=np.float32)
        return C
    ...
    def getG(self, m, l, g):
        ...
        s1=sin(self.phi[0])
        s2=sin(self.phi[1])
        c12=cos(self.phi[0]+self.phi[1])
        s12=sin(self.phi[0]+self.phi[1])
        l1=l[0]
        l2=l[1]
                        
        G=np.array([((m1+m2)*g*l1*c1 +m2*g*l2*c12),
                    (m2*g*l2*c12)], dtype=np.float32)    
        return G
    ...
    def getYM(self, vphi, phi):
        ...
        YM=np.array([[vphi[0], 2*c1*vphi[0], vphi[1], 0.0, 0.0],
    ...
    def getYG(self, phi, g):
        ...
        YG=np.array([[ 0.0, 0.0, 0.0, g*c1, g*c12],
                      [0.0, 0.0, 0.0, 0.0, g*c12]], dtype=(np.float32))
        return YG
    ...
    def step(self, dt, t):
        ...
        G = self.getC(self.m, self.l, self.phi, self.phiD)
